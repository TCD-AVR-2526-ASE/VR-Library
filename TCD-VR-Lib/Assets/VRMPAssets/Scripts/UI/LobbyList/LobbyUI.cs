...
namespace XRMultiplayer
{
    public class LobbyUI : MonoBehaviour
    {
        ...
        /// <summary>
        /// Populates the lobby UI with cached rooms retrieved from the cache service.
        /// Skips rendering if no cached rooms are available.
        /// </summary>
        void AddCachedRoomsToList()
        {
            var rooms = RoomCacheProvider.CacheService?.GetAll();
            int count = rooms?.Count ?? 0;
            Debug.Log($"[RoomCache] Cached rooms: {count}");
            if (count == 0) return;

            foreach (var room in rooms)
            {
                // Optional: show only inactive rooms on relaunch
                // if (room.Status != RoomStatus.Inactive) continue;

                var slot = Instantiate(m_LobbyListPrefab, m_LobbyListParent).GetComponent<LobbyListSlotUI>();
                slot.CreateCachedRoomUI(room, this); // you'll add this method in LobbyListSlotUI
            }
        }

        /// <summary>
        /// Attempts to reconnect to a previously cached room using its stored metadata.
        /// </summary>
        /// <param name="room">The cached room to reconnect to.</param>
        public void RecreateCachedRoom(RoomData room)
        {
            Debug.Log($"[RoomCache] Recreating room: {room.RoomName} ({room.GUID})");

            ToggleConnectionSubPanel(ConnectionSubPanel.ConnectionPanel);
            XRINetworkGameManager.Connected.Subscribe(OnConnected);

            var sessionManager = FindFirstObjectByType<SessionManager>();
            _ = sessionManager.CreateOrJoinFromCache(room);

            m_ConnectionSuccessText.text = $"Reconnecting {room.RoomName}";
        }
    }
}
