    ...
    public class SessionManager : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (sessionType == SessionType.LocalOnly)
                SetupLocalTransport();
            // Catch unexpected disconnects (wifi drop, host ends, etc.)
        ...
        public async Task<ISession> CreateSession(string roomName = null, bool isPrivate = false, int maxPlayers = XRINetworkGameManager.maxPlayers)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    m_CurrentRoomId = guid.ToString(); // this is the ProvidedSessionId

                    StopAllCoroutines();
                    StartCoroutine(PlayConnectionMessage());
                    m_CurrentSession = await MultiplayerService.Instance.CreateOrJoinSessionAsync(guid.ToString(), options);
        ...
        void ConnectedToSession()
        {
            ...
            {
                GUID = m_CurrentRoomId,                    // stable
                SessionID = m_CurrentSession.Id,           // Unity-generated
                JoinCode = m_CurrentSession.Code,          // Unity-generated
                RoomName = m_CurrentSession.Name,
                SceneName = SceneManager.GetActiveScene().name,
                MaxPlayers = m_CurrentSession.MaxPlayers,
                StatusEnum = RoomStatus.Connected,
                LastUpdatedUtc = DateTimeOffset.UtcNow.ToUnixTimeSeconds(),
                Endpoint = null
            };
            _ = RoomCacheProvider.CacheService.Upsert(roomData); // fire-and-forget cache update
        }
        ...
        public async Task LeaveSession()
        {
            if (m_CurrentSession != null)
            {
                // mark inactive before nulling properties
                if (!string.IsNullOrEmpty(m_CurrentRoomId))
                    _ = RoomCacheProvider.CacheService.SetStatus(m_CurrentRoomId, RoomStatus.Dormant); // fire-and-forget
        ...
        /// <summary>
        /// Recreates or joins a multiplayer session using cached room data.
        /// Reuses the stored GUID to ensure consistent session identity.
        /// </summary>
        /// <param name="room">The cached room metadata.</param>
        /// <returns>The joined or recreated session, or null if it fails.</returns>
        public async Task<ISession> CreateOrJoinFromCache(RoomData room)
        {
            if (room == null)
                return null;
            ...
            try
            {
                m_Status.Value = "Reconnecting To Room: " + room.RoomName;

                // Reuse the cached GUID � this is the whole point
                m_CurrentRoomId = room.GUID;
                ...
                m_CurrentSession = await MultiplayerService.Instance
                    .CreateOrJoinSessionAsync(room.GUID, options);

                ConnectedToSession(); // will Upsert instead of inserting
                return m_CurrentSession;
            }
            catch (Exception e)
            {
                string failureMessage = $"Failed to reconnect to {room.RoomName}.";
                Utils.Log($"{k_DebugPrepend}{failureMessage}\n\n{e}", 1);
                OnSessionFailed?.Invoke(failureMessage);
                return null;
            }
        ...
        /// <summary>
        /// Handles local client disconnection events and marks the current
        /// room as Dormant in the cache.
        /// </summary>
        /// <param name="clientId">The ID of the disconnected client.</param>
        void OnClientDisconnected(ulong clientId)
        {
            // Only care about our own client disconnect
