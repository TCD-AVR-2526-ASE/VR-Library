...
public class BookPaginator : MonoBehaviour
{
    ...
    public static bool ProcessBook(Book book)
    {
        if (string.IsNullOrEmpty(book.path) || !File.Exists(book.path))
        {
            UnityEngine.Debug.LogError($"[BookPaginator] File not found: {book.path}");
            // Provide a single error page so the book still spawns
            book.Paginate(new List<string> { $"Could not load book.\nFile not found:\n{book.path}" });
            return false;
        }

        string text = LoadText(book.path);
        return ProcessBookText(book, text);
    }

    public static bool ProcessBookText(Book book, string text)
    {
        if (book == null)
            return false;
